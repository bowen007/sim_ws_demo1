...
function user_geshu(){
     ...
     var length3=$(".ul_3 > li").length;
     $(".n_geshu_1").text(length1);
     $(".n_geshu_2").text(length2);	
     $(".n_geshu_3").text(length3);
}
user_geshu()
//alert(length1)
...
function saysay(){
	 ...
	 if($(".list").length<1){
	  ...
	  if($("#texterea").val()){
      console.log('in send');
      console.log(ing_user[10]);
      var ms = {message_content:$("#texterea").val(),message_to:ing_user[10]};
      console.log(ms)
...
function has_message(evt){
   var data = eval('(' + evt.data + ')');
   var t=new Date().toLocaleTimeString();//当前时间

   ({
       'user':function(){
          $("#user_con"+0).append('<div class="my_say_con"><font color=\"#0000FF\">'+data['message_from']+t+"</font><p><font color=\"#333333\">"+trim2(trim($("#texterea").val()))+'</font></p></div>');
	      $("#right_mid").html(data['message_content']);//右边显示刚发送的文字
	      $("#texterea").val("");
	      $(".my_show").scrollTop($(".con_box").height()-$(".my_show").height());//让滚动滚到最底端
      }
...
window['dandan']['has_message']=has_message;
...
function has_message(event){
   ...
   ({
        'sys':function(){
        console.log(data['message_from']);
        console.log(data['message_content']);
        var to = data['message_to'];
          $("#user_contitle_user"+to).append('<div class="my_say_con"><font color=\"#0000FF\">'+data['message_from']+t+"</font><p><font color=\"#333333\">"+trim2(trim(data['message_content']))+'</font></p></div>');
	      $("#right_mid").html(data['message_content']);//右边显示刚发送的文字
	      $("#texterea").val("");
	      $(".my_show").scrollTop($(".con_box").height()-$(".my_show").height());//让滚动滚到最底端
